...
`esy-afl` is [`afl`](https://github.com/google/AFL), packaged for [`esy`](https://esy.sh/).
...
`esy-afl` can be used from both NPM and directly from Github.

### From NPM 

`esy-afl` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-afl` to install it, or specify it in
...
```diff
  "dependencies": {
+   "esy-afl": "esy-packages/esy-afl#<commit hash>"
  }
```
